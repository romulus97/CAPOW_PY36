...
PGE_dams = list(df_hydro.loc[:,'Balch 1':])
...
upper_gen = pd.read_excel('upper.xlsx',header =0)
...
for dam in model:
    if dam in PGE_No_Data_Dams:
        pass
    elif dam in PGE_Storage:
        pass
    else:
...
#                hydro_weekly = np.append(hydro_weekly,np.sum(hydro_ts[i*7:i*7+7]))
#                flow_weekly = np.append(flow_weekly,np.sum(flow_ts[i*7:i*7+7]))
#                hydro_weekly = hydro_weekly[np.logical_not(np.isnan(hydro_weekly))]
#                
            # calculate dam "efficiency" (MWh/unit inflow)
            water = np.sum(flow_ts[:])
            power = np.sum(hydro_ts[:])
            ...
            exec("np.savetxt('FNF_%s.txt',Save_Results,delimiter = ' ')" %(s))
            # re-run to get plot
    #        exec("plt.figure(%d)" %(z))
