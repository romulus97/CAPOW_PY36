...
#=======
# allocate output
outflows_all = np.full((T+2,n_res,7),np.nan) #we can fill these in later, or make them empty and 'append' the values
hydropower_all = np.full((T+2,n_HPres,7), np.nan)
volumes_all = np.full((T+2,n_res,7),np.nan)
elevations_all = np.full((T+2,n_res,7),np.nan)
cp_discharge_all = np.full((T+2,(n_cp),7),np.nan)
...
#       #forecast day? if not, take beginning storage from previous time step
        if t<=1 and fd < 1:
            ...
            [COT_volume,COT_elevation] = inner.UpdateVolume_elev (COT, COT.dataIN[t,fd], COT_outflow, volumes_all[t-1,COT.ID-1])
            ...
            if doy==120:
                waterYear = inner.UpdateReservoirWaterYear(doy,s,volumes_all[]) #function missing
