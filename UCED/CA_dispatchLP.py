...
def SysCost(model):
    ...
    return fixed + coal1 + coal2 + coal3 + gas1_1 + gas1_2 + gas1_3 + gas1_4 + gas2_1 + gas2_2 + gas2_3 + gas2_4 + gas3_1 + gas3_2 + gas3_3 + gas3_4 + sce_imports1 + sce_imports2 + sce_imports3 + sdge_imports1 + sdge_imports2 + sdge_imports3 + pgev_imports1 + pgev_imports2 + pgev_imports3 + oil1 + oil2 + oil3 + psh1 + psh2 + psh3 + slack1 + slack2 + slack3 + starts + exchange + nreserves + sreserves
...
def HydroC1(model,i):
    ...
    r1 = sum(model.srsv['PGEV_hydro',i] for i in model.h1_periods)
    n1 = sum(model.nrsv['PGEV_hydro',i] for i in model.h1_periods)
    return m1 + m2 + m3 + r1 + n1 <= model.HorizonPGE_valley_hydro[1]
...
def HydroC2(model,i):
    ...
    r1 = sum(model.srsv['PGEV_hydro',i] for i in model.h2_periods)
    n1 = sum(model.nrsv['PGEV_hydro',i] for i in model.h2_periods)
    return m1 + m2 + m3 + r1 + n1 <= model.HorizonPGE_valley_hydro[2]
...
def HydroC3(model,i):
    ...
    r1 = sum(model.srsv['SCE_hydro',i] for i in model.h1_periods)
    n1 = sum(model.nrsv['SCE_hydro',i] for i in model.h1_periods)
    return m1 + m2 + m3 + r1 + n1 <= model.HorizonSCE_hydro[1]
...
def HydroC4(model,i):
    ...
    r1 = sum(model.srsv['SCE_hydro',i] for i in model.h2_periods)
    n1 = sum(model.nrsv['SCE_hydro',i] for i in model.h2_periods)
    return m1 + m2 + m3 + r1 + n1 <= model.HorizonSCE_hydro[2]
