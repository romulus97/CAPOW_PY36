...
# Import historical tmeperature data
    ...
    df_temp0=pd.read_csv('Historical_weather_analysis/synthetic_weather_data_{}-{}.csv'.format(yearbeg, yearend))
    his_temp_matrix = np.column_stack((df_temp0.iloc[:,1].values,(df_temp0[cities].values)*(9/5)+32)) # Convert temperatures to Fahrenheit 
    df_temp=pd.DataFrame(his_temp_matrix);
    df_temp.columns=["Time"] + [x.title()[:-2] for x in cities];
    
    # Import calender 
    calender=pd.read_csv('Synthetic_streamflows/Calender.csv',header=None)
    ...
    # Simulation data
    sim_weather=pd.read_csv('Synthetic_weather/synthetic_weather_data_{}-{}_{}.csv'.format(yearbeg, yearend, sim_years))
    ...
    HDD_sim = 65-sim_temperature; HDD_sim[HDD_sim<0]=0;
    CDD_sim = sim_temperature-65; CDD_sim[CDD_sim<0]=0;
    ...
    for j in range(0,num_cities):
        annual_HDD_sim[:,j]=np.sum(HDD_sim[:,j].reshape(int(len(HDD_sim)/365),365),axis=1)
        annual_CDD_sim[:,j]=np.sum(CDD_sim[:,j].reshape(int(len(CDD_sim)/365),365),axis=1)
    ...
    HDD = 65-his_temp_matrix[:,1:]; HDD[HDD<0]=0;
    CDD = his_temp_matrix[:,1:]-65; CDD[CDD<0]=0;
    ...
    for j in range(0,num_cities):
        annual_HDD[:,j]=np.sum(HDD[:,j].reshape(int(len(HDD)/365),365),axis=1)
        annual_CDD[:,j]=np.sum(CDD[:,j].reshape(int(len(CDD)/365),365),axis=1)
    ...
    # Load relevant streamflow data (2005-2034)
    BPA_streamflow=pd.read_csv('Synthetic_streamflows/synthetic_streamflows_BPA_{}-{}.csv'.format(yearbeg, yearend),header=0)
    Hoover_streamflow=pd.read_csv('Synthetic_streamflows/synthetic_discharge_Hoover_{}-{}.csv'.format(yearbeg, yearend),header=0)
    CA_streamflow=pd.read_csv('Synthetic_streamflows/synthetic_streamflows_CA_{}-{}.csv'.format(yearbeg, yearend),header=0)
    Willamette_streamflow=pd.read_csv('Synthetic_streamflows/synthetic_streamflows_Willamette_{}-{}.csv'.format(yearbeg, yearend),header=0)
    
    
    # %% offsetting the streamflow to remove the negative streamflow records
    # min_BPA=BPA_streamflow.iloc[:,3:].values.min(0); 
    # min_Hoover=Hoover_streamflow.iloc[:,3:].values.min(0);
    # min_CA=CA_streamflow.iloc[:,3:].values.min(0); 
    # min_Willamette=Willamette_streamflow.iloc[:,3:].values.min(0); 
    # min_streamflows=np.concatenate((min_BPA,min_Hoover,min_CA,min_Willamette))
    
    # BPA_streamflow.iloc[:,3:]=BPA_streamflow.iloc[:,3:]-min_BPA
    # Hoover_streamflow.iloc[:,3:]=Hoover_streamflow.iloc[:,3:]-min_Hoover
    # CA_streamflow.iloc[:,3:]=CA_streamflow.iloc[:,3:]-min_CA
    # Willamette_streamflow.iloc[:,3:]=Willamette_streamflow.iloc[:,3:]-min_Willamette
    ...
    # %% transfer or normalize data
    # added_valueB=abs(np.min((B)))+5
    # log_B=np.log(B+abs(added_valueB))
    # B=log_B;
    ...
    mean=np.mean(B_std,axis=0)
    cov=np.cov(B_std,rowvar=0)
    runs=sim_years*5
    ...
    for i in range(0,y):
        ...
        cond0=np.where(T[:,i]<0)[0];
        cond1=np.where((T[:,i]>=0) & (T[:,i]<=1))[0];
        cond2=np.where(T[:,i]>1)[0];
        T_all[cond0,i]=np.percentile(B[:,i],0*100,axis=0)*(1+T[cond0,i])
        T_all[cond1,i]=np.percentile(B[:,i],T[cond1,i]*100,axis=0)
        T_all[cond2,i]=np.percentile(B[:,i],1*100,axis=0)*T[cond2,i]
    ...
    # added_value=abs(np.min((B)))+5;
    # B_add=abs(B.min(0))+5
    # B_log=np.log(B+abs(B_add))
    # # B_std = ss.fit_transform(B)
    # B_std = B_log
    
    
    # from statsmodels.graphics.gofplots import qqplot
    # ps=np.zeros((B.shape[1],2))
    # for i in range(0,B.shape[1]):
    #     k2, ps[i,0] = st.normaltest(B[:,i])
    #     k2, ps[i,1] = st.normaltest(B_std[:,i])
    # #     # ps2=np.where(ps[i,:]==max(ps[i,:]));
    # indices = ps.argmax(axis=1)
    
    # ps2=np.where(indices==0)
    ...
    # T_all=np.exp(N)-B_add
    
    # # T_all[:,ps2]=T_all0[:,ps2]
    
    
    # a=0
    # T_all = ss.inverse_transform(N)
    # T_all = N
...
#     need_to_remove=np.where(np.min(Y,axis=0)==np.max(Y,axis=0));
#     
...
# =============================================================================
    ...
    Temperature_weights0=pd.read_excel('Synthetic_streamflows/city_weights.xlsx',header=0)
    Temperature_weights=Temperature_weights0[Fraction_calculation_cities] #reorder the weight 
    ...
    weighted_T=np.matmul(fraction_hist_temp_matrix,np.transpose(Temperature_weights.values));
    ...
    weighted_T_sim=np.matmul(fraction_sim_matrix,np.transpose(Temperature_weights.values));
    ...
    hist_T=np.transpose(np.reshape(weighted_T, (hist_years,365)))
    sim_T=np.transpose(np.reshape(weighted_T_sim, (sim_years,365)))
    
    # hist_T=np.zeros((365,hist_years))
    # sim_T=np.zeros((365,sim_years))
    # for i in range(0,hist_years):
    #     hist_T[:,i] = weighted_T[i*365:365+(i*365),0]
    ...
    for i in range(0,sim_years):
     ...
     # %% recovering the streamflow

        
    # output_BPA0=output_BPA;
    # output_Hoover0=output_Hoover;
    # output_CA0=output_CA;
    # output_WI0=output_WI;
    
    # output_BPA=output_BPA0+min_BPA; 
    # output_Hoover=output_Hoover0+min_Hoover;
    # output_CA=output_CA0+min_CA;
    # output_WI=output_WI0+min_Willamette;
    # %%
    ...
    ###############################################################################
    # Output
    np.savetxt('Synthetic_streamflows/synthetic_streamflows_FCRPS_{}-{}_{}.csv'.format(yearbeg, yearend, sim_years),output_BPA,delimiter=',')
    np.savetxt('Synthetic_streamflows/synthetic_streamflows_TDA_{}-{}_{}.csv'.format(yearbeg, yearend, sim_years),TDA[:,1],delimiter=',')
    np.savetxt('Synthetic_streamflows/synthetic_discharge_Hoover_{}-{}_{}.csv'.format(yearbeg, yearend, sim_years),output_Hoover,delimiter=',')
    CA=pd.DataFrame(output_CA,columns=name_CA)
    CA.to_csv('Synthetic_streamflows/synthetic_streamflows_CA_{}-{}_{}.csv'.format(yearbeg, yearend, sim_years))
    Willamatte_Syn=pd.DataFrame(output_WI,columns=name_Will)
    Willamatte_Syn.to_csv('Synthetic_streamflows/synthetic_streamflows_Willamette_{}-{}_{}.csv'.format(yearbeg, yearend, sim_years))
    
    ## Create the datelist from the start_year to end_year
    #simulation_period_start=1901
    # file_start_date=pd.to_datetime(simulation_period_start,format='%Y');
    # file_end_date=pd.to_datetime(simulation_period_start+sim_years,format='%Y');
    
    file_start_date=str(simulation_period_start) +'-01-01';
    file_end_date=str(simulation_period_start+sim_years-1) +'-12-31';
    # dates_as_datetime = pd.date_range(start=file_start_date, end=file_end_date, freq='D')
    dates_as_datetime = pd.period_range(start=file_start_date, end=file_end_date, freq='D')
    
    ## Leap the date with 29 days in Feb, 365 records are simulated for each year
    date_mask = (dates_as_datetime.month==2) & (dates_as_datetime.day==29);
    B = np.empty((1,len(name_CA))); B.fill(np.nan);
    F1=np.zeros((len(dates_as_datetime),len(name_CA)));
    F1[date_mask,:]=B;
    F1[~date_mask,:]=output_CA
    
    ## Merge the datetime and data dataframe % important, see read_new_fnf_data function in main.sim(sim_years)
    df_leap0 = pd.DataFrame({'datetime': dates_as_datetime});
    df_leap1 = pd.DataFrame(F1,columns=name_CA);
    df_leap=pd.concat([df_leap0, df_leap1], axis=1)
    
    #write CA synthetic flows to ORCA file
    df_leap1.to_csv('Synthetic_streamflows/forecast_flows_{}-{}_{}.csv'.format(yearbeg, yearend, sim_years))
      
        
    return None


# sim_years_input=3; simulation_period_start=1901; yearbeg=2005; yearend=2034;
# synthetic(sim_years_input,simulation_period_start,yearbeg,yearend)